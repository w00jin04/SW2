...
void loop() {
  ...
  if(dist_ema <= 180){
    ...
    double t = dist_ema - 180;  // 거리값에서 최소값(180)을 뺌
    double ratio = t / 180.0;   // 거리 범위를 0~1의 비율로 변환
    int duty = _DUTY_MIN + ratio * (_DUTY_MAX - _DUTY_MIN);  // 펄스 폭 비례식
    myservo.writeMicroseconds(duty);  // 거리값에서 최소값(180)을 뺌
  }
